...
export default function Login() {
  ...
  return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#f5ede0'}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@700&family=DM+Sans:wght@400;600&display=swap');`}</style>
      <div style={{background:'#fdf8f2',border:'1px solid #d8c4a8',borderRadius:16,padding:'40px 36px',width:380,boxShadow:'0 8px 40px rgba(44,24,16,0.1)',fontFamily:"'DM Sans',sans-serif"}}>
        ...
        <form onSubmit={handleLogin}>
          <div style={{marginBottom:16}}>
            ...
            <input type="email" value={email} onChange={e=>setEmail(e.target.value)} required placeholder="your@university.ac.uk"
              style={{width:'100%',padding:'10px 12px',border:'1.5px solid #d8c4a8',borderRadius:8,fontSize:13,background:'#f5ede0',outline:'none',boxSizing:'border-box',fontFamily:'inherit'}}/>
          </div>
          ...
          <button type="submit" disabled={loading}
            style={{width:'100%',padding:12,background:loading?'#8a5c3c':'#1c2e28',color:'#f2e4d0',border:'none',borderRadius:8,fontSize:14,fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>
