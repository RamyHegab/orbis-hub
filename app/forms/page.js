...
export default function Page() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const { data: p } = await supabase.from('users').select('full_name,role').eq('id', session.user.id).maybeSingle()
      if (p) setProfile(p)
    })
  }, [])
  return (
    <div className="shell">
      ...
      <main className="main">
        <div className="topbar">
          <div className="tb-bc"><span className="bc-cur">Coming Soon</span></div>
        ...
        <div className="content">
          <div className="view active" style={{padding:'40px 36px'}}>
            <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:32,fontWeight:700,color:'#2c1810',marginBottom:12}}>Coming Soon</div>
            <div style={{fontSize:13,color:'#8a5c3c',marginBottom:24}}>This section is being connected to the live database.</div>
